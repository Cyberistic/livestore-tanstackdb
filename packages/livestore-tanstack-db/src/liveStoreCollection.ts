...
export type CoerceRow<TIn extends LiveStoreRow, TOut extends LiveStoreRow> = (row: TIn) => TOut;
...
export interface LiveStoreCollectionUtils<T extends LiveStoreRow = LiveStoreRow> extends UtilsRecord {
...
> {
   ...
   */
  onInsert?: (
    params: InsertMutationFnParams<T, string, LiveStoreCollectionUtils<T>>,
  ) => Promise<void>;
   ...
   */
  onUpdate?: (
    params: UpdateMutationFnParams<T, string, LiveStoreCollectionUtils<T>>,
  ) => Promise<void>;
   ...
   */
  onDelete?: (
    params: DeleteMutationFnParams<T, string, LiveStoreCollectionUtils<T>>,
  ) => Promise<void>;
}
...
export function liveStoreCollectionOptions<T extends LiveStoreRow>(
  ...
  const wrappedOnInsert = onInsert
    ? async (params: InsertMutationFnParams<T, string, LiveStoreCollectionUtils<T>>) => {
        ...
        const mutations = (params.transaction as { mutations: ReadonlyArray<{ modified: T }> })
          .mutations;
