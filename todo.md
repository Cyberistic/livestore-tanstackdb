...
> Source of truth for _all_ work on this stack. Mirrors the dream-list
...
| 0.3 | `useTable(name).insert/update/delete` that round-trips through LiveStore AND oRPC        | ✅     | Shipped. `useTable('Todo', { rpc: { client, config } })` wires the oRPC config into `createMutations` (Tier 0.6). Direct calls to `todosCollection.update(id, draft => {...})` and `todosCollection.delete(id)` fire the auto-classified oRPC procedures (`posts.complete`, `posts.delete`, etc.) in addition to committing the LiveStore event. Both apps exercise this path: `useTable` → `onToggle` → `todosCollection.update(...)` → oRPC + LiveStore. **Adapter hardening**: `createORPCAdapter` now wraps each resolved procedure function in a plain arrow wrapper so subsequent property access on the oRPC Proxy (e.g. by `console.log`, React reconciler, devtools) cannot extend its procedure path. Required `router: Record<string, unknown>` option to enumerate namespaces (oRPC clients are Proxies with no enumerable own keys). |
...
- **Linting**: `bunx oxlint` and `bunx oxfmt` both pass (0 warnings, 0 errors) after the cleanup pass. Keep CI green by running them before commits.
- **`posts.complete` idempotency** (tanstack-start-orpc example): the example's in-memory `db.rows` is separate from LiveStore's D1-backed event log, so a todo created in a previous session (or via sync) isn't in the server's map. `completePost` upserts from the input shape so toggling always succeeds. `deletePost` still throws `ORPCError('NOT_FOUND')` to demonstrate proper oRPC error typing through the server's `onError` interceptor and the client's `rpc.onError` callback.
- **`rpcConfig.complete`** pins `{ event: 'todoCompleted' }` so `classifyProcedure` routes it to the UPDATE bucket only — without this it falls into the `['insert', 'update']` upsert default and fires on every new row, racing the create RPC.
